...
def run_canvis(field,ID,run,verbose=False,debugmode=False):
    ...
    path ='/fred/oz100/pipes/DWF_PIPE/MARY_SUB/'+field+'_'+ date +'_mlt1_*/*ccd' + ccd_num + '*.fits'
    ...
    for i, (key, (path, pixx, pixy)) in enumerate(mydic.items()):
        ...
        path_cutout = '/fred/oz100/CANVIS/cand_images/'+ run +'/cand_'+format(ID, '05')+'_'+ field +'_'+ run +'/cand_'+format(ID, '05')+'_'+run+'_sub_cutout_'+format(i, '03')  
        print(path_cutout)
    ...
    writer = imageio.get_writer(str(path_cutout)  + str(ID) +'SUB_VIDEO.gif', fps =3)
    video_loc = str(path_cutout)  + str(ID)+'SUB_VIDEO.gif'
    for i in files:
